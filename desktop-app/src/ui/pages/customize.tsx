...
function Customize() {
    ...
    const renderKeyBindings = (keybinding: typeof Key[keyof typeof Key][]) => {
        ...
        // Create 3 pills (some may be empty)
        for (let i = 0; i < 3; i++) {
    ...
    const renderAnalogKeyBindings = (keybinding: typeof Key[keyof typeof Key][], direction: string) => {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            pills.push(
                <div
                    key={i}
                    className={`w-1/4 h-10 rounded-md flex items-center justify-center ${key ? 'bg-neutral-900 border border-yellow-200' : 'bg-neutral-900 border border-neutral-400'}`}
                >
                    {KeyNum[key] || ''}
                </div>
            );
                                ...
                                <div className="mb-2 text-sm font-medium">
                                    <span className={`px-3 py-1 rounded-full ${
                                        mapping.target.type === 'keyboard'
                                            ? 'border-blue-100 bg-neutral-800 text-blue-300'
                                            : mapping.target.type === 'mouseClick'
                                            ? 'border-green-100 bg-neutral-800 text-green-300'
                                            : mapping.target.type === 'analogKeyboard'
                                            ? 'border-yellow-100 bg-neutral-800 text-yellow-200'
                                        ...
                                        {mapping.target.type === 'keyboard'
                                            ...
                                            : mapping.target.type === 'mouseClick'
                                            ? 'Mouse Click'
                                            : mapping.target.type === 'mouseMotion'
